            ...
            <div className="server light">
                <div className="server-split">
                    {Object.keys(sources).slice(0, 4).map((key, i) => (
                        <Tooltip key={key} title={sources[key].tooltip} className={`ss-item ${!!sources[key].icon && 'movieslaysource'}
                            ${i === 3 && 'right-corners'}
                            ${i === 0 && 'left-corners'} ${source == key && 'active'}`} onClick={() => {
                                setSource(key);
                            }}>
                            {!!sources[key].icon ? <i>
                                <img src={sources[key].icon} style={{ width: '25px', height: '25px', margin: '0 auto' }} />
                            </i> : <i className={`fa-solid fa-dice-${['one', 'two', 'three', 'four'][i]}`}></i>
                            }
                        </Tooltip>
                    ))}
                </div>
